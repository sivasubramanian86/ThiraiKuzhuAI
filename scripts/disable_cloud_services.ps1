...
try {
    ...
    if ($serviceCheck) {
        Write-Host "  📉 Scaling Cloud Run service '$ServiceName' to 0 instances..." -ForegroundColor Yellow
        gcloud run services update $ServiceName `
            ...
            --max-instances=0 `
            --quiet
        Write-Host "  ✓ Cloud Run instances frozen at 0." -ForegroundColor Green
    } else {
        Write-Host "  ℹ️ Cloud Run service '$ServiceName' not deployed yet." -ForegroundColor Gray
    }
} catch {
...
Write-Host "  🔒 Disabling billable auxiliary APIs..." -ForegroundColor Yellow
$servicesToDisable = @(
